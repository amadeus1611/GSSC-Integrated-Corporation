...
- `expira-system/console/index.html` is the EXPIRA Console, a single-file artifact. It is live at https://claude.ai/artifact/LcLXJASXhWZ56g74sHVRxt and currently at v44. It is built from `src/`; never edit it by hand.
...
- `expira-system/console/REBUILD_PLAN.md` is the v41 clean rebuild plan (shipped). `INSPECTOR_PLAN.md` is v42: synced disclosures and the Details rail. v43 made the sidebar, rail and Dispatch moves one continuous move, with the chat on the panel's own curve (`setFold` in `units/shell/shell.js`, margins in `shell.css` and `sheet.css`). `SUPER_PLAN.md` is v44: the blur bloom in `core/pour.js`, faster timings, and the full-screen map's Details, Weighing and Log pane (`FSP` and `fsTab` in `units/sheet/sheet.js`). `SUPER_PLAN_45.md` is v45, planned and not yet built: finishing the bench sidebar draft (Isolate and an always-on path bar, folders that show their state, Recently Deleted, moving many at once, touch, search inside chats, scale, screen readers) and moving it into `src/`.
- `expira-system/DESIGN_ENGINE.md` holds the design and motion canon. **Read it before any design or motion work**, and add to its change log after each iteration.
  - §4 **EXPIRA × macOS** is the house style for the Console and every EXPIRA derivative: the file-system logic, one curve with focus in and focus out, and the improved near-black stack. The GSSC kernel templates (quotation master, company profile, contracts, resolutions) keep their own look and colour logic.
