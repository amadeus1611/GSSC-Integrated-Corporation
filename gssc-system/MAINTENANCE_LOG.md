...
President ("I approve it"): cut the release here and bring the JSON
container to its final form. He updates the Copilot/365 copy afterwards.

**Source:** Canva design `GSSC_Native_Logo-Pack` (DAHLkhKRfrg, 5 pages,
2000×2000, transparent PNG), kept in `release/logo_pack/`. Mapping: p1 mark
→ `seal`; p2 stacked lockup → `watermark`; p3 wordmark → new `wordmark`
(library asset, `template_use: false`); p4 horizontal lockup → `header`;
p5 full-name lockup → `signature`. Measured colours match the palette:
wordmark navy #0d2463 = `navy`, and the bar behind "Integrated" #e6d4a7 =
`gold_light`.

**Done:**
- `package/GSSC_Master_Package_v2_17_0.json`, cut by
  `release/cut_release_2_17_0.py` (deterministic).
  - Assets are trimmed to the ink and sized at about 5× their render size:
    header 1062×190, seal 235×240, signature 1000×147, watermark
    1200×1491, wordmark 1200×252.
  - 03_brand gains `logo_system`; 02_governance gains `signatory_policy`
    (proposal 1); protocol 2.7 changes §24, §03/§28C and §43.
  - 13_changelog: 2.17.0 is current, adopted by Duke; 2.9.0 and 2.16.1
    are superseded. `adoption_record` is populated (board_reference is
    blank).
  - Every hash is recomputed and `docs/` re-extracted.
  - Preflight PASS.
- Runtime 2.0 (proposals 2–5 plus the library-asset rule). `build.json`
  carries no audit exceptions any more: all 6 documents pass all 14
  audits, and all 6 were rebuilt with the new logos and checked rendered.
- `GSSC_Master_Package_v2_16_1.json` removed from the working tree (it
  remains in git history; the release script reads it from there if
  re-run).
- The package grew from 1.17 MB to 1.81 MB, mostly the watermark
  (523 KB). Each document grew by about the same amount.

**Open:** board resolution reference for the adoption; protocol prose for
Sections 44–46; Duke loads 2.17.0 into 365/Copilot.
