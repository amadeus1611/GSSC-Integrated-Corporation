...
- **v43** (2026-09-26, after Amadeus's notes on the sidebar, card stutter and repeated Enter):
  - layout moves glide: the sidebar fold, the Details rail and the Dispatch change the layout once, then everything plays back on transforms (the sidebar slides, the header titles and fold button FLIP, the chat column and composer glide). The column keeps the width it had for the move and re-wraps once as it lands, so no line reflows mid-move; the scroller's clip box widens for the move so nothing is cut at its edge, and the sidebar sits above so passing lines slide under it. No margin transitions remain;
