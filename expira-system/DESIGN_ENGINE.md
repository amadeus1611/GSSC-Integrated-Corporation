...
- **Whole-list moves are one field, arranged around an anchor** (stepping into a folder, stepping out, searching; after Material's choreography rules and Amadeus's "react with the other files and folders"). The anchor is the folder you step into or out of, or the search field.
  ...
  - **Stepping out anchors on the folder you were in.** The anchor is the deepest folder on the path between where you land and where you were that the list can show: the folder itself whenever its parents are open, otherwise its nearest visible ancestor. Only that folder and its own contents travel; every other row stays where it belongs and simply unfolds around them. If the folder lands off screen, the list first scrolls it into view (a third of the way down), as the Finder keeps the folder you came out of in sight.
