...
- **v43** (2026-09-26, after Amadeus's notes on the sidebar, card stutter and repeated Enter):
  - layout moves glide: the sidebar fold, the Details rail and the Dispatch change the layout once, then everything plays back on transforms (the sidebar slides, the header titles and fold button FLIP, the chat column and composer glide). While it moves, the column always has the narrower of its two widths: a column that narrows re-wraps at once, as the panel starts to arrive, and one that widens re-wraps once as it lands, so no line reflows mid-move and the column never passes under a panel. On a wide window the rail and the Dispatch come in from past the edge, never lying see-through over the column; the scroller's clip box widens for the move so nothing is cut at its edge. No margin transitions remain;
  - the sidebar header keeps the fold button's height after the button leaves it, so the items below never jump up;
  - the Dispatch builds inside the glide (one layout, not two) and its maps wake after the panel lands;
