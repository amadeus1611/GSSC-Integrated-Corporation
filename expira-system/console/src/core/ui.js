...
function disclose(host,cls,on,body,inv){const isOn=()=>host.classList.contains(cls)!==!!inv,set=v=>host.classList.toggle(cls,inv?!v:v);
 if(isOn()===on&&!host._dc)return;host._want=on;const tok=host._dc={};if(host._fa){host._fa.cancel();host._fa=null}
 /* what sat below moves from where it was to where it lands: transform only */
 const glide=()=>{const below=[];for(let n=host,k=0;n&&k<4&&n.id!=="scroll";n=n.parentElement,k++)for(let s=n.nextElementSibling;s;s=s.nextElementSibling)below.push(s);
  const was=below.map(s=>s.getBoundingClientRect().top);return()=>below.forEach((s,i)=>{const d=was[i]-s.getBoundingClientRect().top;if(Math.abs(d)>.5)s.animate([{transform:`translateY(${d}px)`},{transform:"none"}],{duration:MO.move,easing:MO.spring})})};
 if(reduce||!body||!host.isConnected){set(on);host._dc=null;host._want=null;return}
 if(on){const land=glide();set(true);host._dc=null;host._want=null;land();return}
 const fa=host._fa=body.animate([{opacity:1},{opacity:0}],{duration:MO.exit,easing:MO.soft,fill:"forwards"});fa.finished.then(()=>{if(host._dc!==tok)return;host._dc=null;host._fa=null;
  const land=glide();set(false);fa.cancel();host._want=null;land()}).catch(()=>{})}
