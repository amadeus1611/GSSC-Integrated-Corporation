...
setInterval(()=>{if(promptEl.value||document.activeElement===promptEl||busy||!sample||reduce)return;const p=$("#phx");p.classList.add("out");setTimeout(()=>{phI=(phI+1)%PH.length;p.textContent=PH[phI];p.classList.remove("out")},500)},5200);/* Enter sends, or adds to the run while one is live. An empty Enter during a run does nothing: only the Stop button or Esc stops it,
   so a repeated Enter can neither start a second run nor cut the first one short. */
promptEl.addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey&&!e.isComposing){e.preventDefault();if(e.repeat||(busy&&!promptEl.value.trim()))return;$("#form").requestSubmit()}});
