...
function setBusy(on){if(busy!==on&&!reduce){const b=$("#send");b.animate([{transform:"none",borderRadius:"4px"},{transform:"scale(.7,.84)",borderRadius:"50%",offset:.34},{transform:"scale(1.07,.95)",borderRadius:"6px",offset:.68},{transform:"none",borderRadius:"4px"}],{duration:MO.enter,easing:MO.out})}busy=on;app.classList.toggle("busy",on);$("#runPill").hidden=!on;const s=$("#send");s.setAttribute("aria-label",on?"Stop":"Send");s.dataset.tip=on?"Stop  Esc":"Send";
