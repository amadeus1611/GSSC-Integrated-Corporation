...
const dsp=$("#dsp");/* fill, when given, builds the panel before it opens */
const showDsp=(o,fill)=>{const ch=!!o!==dsp.classList.contains("open");fill&&fill();dsp.classList.toggle("open",o);app.classList.toggle("dsp-open",o);$("#dspBtn")?.setAttribute("aria-expanded",String(!!o));if(o&&ch)setTimeout(()=>FM.forEach(m=>m.go()),reduce?30:MO.move)/* the maps wake once the panel has landed, so their first frames never share the slide */};
...
/* with nothing filed yet the Dispatch still opens, and says what will appear here and how to get it */
function emptyDispatch(){$("#dspNo").textContent="";$("#dspB").innerHTML=`<div class="dx-empty"><div class="kicker bi"><span class="cap">Nothing filed yet</span></div><h2 class="bi" style="animation-delay:.04s">The Dispatch is written as EXPIRA works.</h2><p class="bi" style="animation-delay:.08s">Send a brief and this page fills with the run: the plan, each desk's notes, the Arbiter's verdicts, the sources and the map.</p><button class="btn2 bi" type="button" data-dx-go style="animation-delay:.12s">Write a brief</button></div>`}
$("#dspB").addEventListener("click",e=>{if(e.target.closest("[data-dx-go]")){showDsp(false);promptEl.focus()}});
function openLatest(){if(busy){showDsp(true);return}const c=cur||chats.find(x=>!x.example),k=c?c.turns.map(t=>t.role).lastIndexOf("assistant"):-1;if(k<0){showDsp(true,emptyDispatch);return}if(c!==cur)open(c);showDsp(true,()=>recordedDispatch(c.turns[k].work||{ms:0,steps:[],feed:[],firewall:"off"},c,c.turns[k-1]?.ts||c.turns[k].ts))}
