...
- [x] Phase 4: review PASS, performance, accessibility (2026-09-26)
  - **Review (round 1): FAIL, all findings fixed.** A crafted library import could run script through feed lines (now every feed line is rendered through `feedH`, which lets only `<em>` through; `qa/store.js` proves a crafted feed stays text). Storage: the newer copy of a chat wins on hydrate and import, deletions hold across devices (tombstones in `gone`), a chat over 250 KB (measured in bytes) stays local and its db copy is removed, a run in flight keeps its chat object, and failed writes back off to 60 s. The primary signatory comes from the kernel's 02_governance (the officer titled President), not a built-in name. Motion: the mercury slide, the lightbox, the greeting, the thumbnails, the composer's focus lift and every disclosure (run card, log, exhibits, folders) now move only transform and opacity; the ready light breathes three times and rests; the map tooltip is measured by an observer, not in the draw loop. The shared `runs` log no longer stores the brief.
  - **Review (rounds 2 and 3): PASS.** Round 2 found that an imported chat could carry non-numeric counts and image URLs that were not data URLs, and that a click mid-close toggled the wrong way; imports are now normalised (`cleanChat`: numbers, a safe `v`, data-URL images, no `__proto__`) and disclosures track where they are heading (`discToggle`). Round 3 passed; its two notes are applied: the close fade runs on `--t-exit`, and opening a disclosure glides what sits below as closing does.
  - **Performance (`qa/perf.js`): pass.** A Chrome trace of a full mock run (`qa/out/perf/trace.json`); no frame over 50 ms and no long task while settled on the start page, the example, after a run, with the card map open and with the Dispatch open; no page rAF callbacks while settled; every map asleep; a map scrolled off-screen mid-run stops drawing.
  - **Accessibility (`qa/a11y.js`, axe-core from outside the repo): pass but one moderate item.** Fixed: `lang`, a `main` landmark and an `h1`, text-safe gold (`--gold-ink`) for small gold text, quiet text on hovered rows, message actions no longer dimmed below AA, the account menu's theme radios, the palette as a combobox and listbox (and inert when closed), labelled scroll regions, table header names, the rating glyphs and the map group, and tooltips hidden from assistive tech (controls carry their own labels). A keyboard walk finds a focus indicator on every stop (the prompt's is the composer's own). Reduced motion leaves nothing looping. Left: `heading-order` is now satisfied by an off-screen "The answer" heading; the placeholder's contrast is only low mid-fade.
- [x] Phase 5: shipped as v41 (2026-09-26): republished to the live artifact (version 51, capabilities carried forward: Exa, sample, db, downloads), DESIGN_ENGINE change log and CLAUDE.md updated for the `src/` workflow
- [ ] **Gate B passed** (Duke reviews the live console)

### Resume here

- **State (2026-09-26, session 3):** v41 is shipped and live; every phase is done and pushed. Smoke reports 0 errors in both themes; `qa/store.js`, `qa/perf.js`, `qa/hovers.js` and `qa/a11y.js` pass.
- **Next: Gate B.** Duke reviews the live console. Fixes from that review go into the owning unit under `src/`, then build, test and republish as in CLAUDE.md. Ask before merging PR #1.
- **For Duke at Gate B:** whether to declare the `user` capability, which turns on private per-viewer storage in the artifact's db (until then the library stays in each browser's local storage). Note that the shared `runs` log no longer stores the brief.
